...
if ($isDeployed)
{
	...
	try
	{
		...
		#$suppressedOut = RunLinuxCmd -username $user -password $password -ip $dtapServerIp -port $dtapServerSshport -command "$python_cmd start-server.py -i1 -p $dtapServerUDPport -u yes && mv Runtime.log start-server.py.log -f" -runAsSudo
		...
		if(IsIperfServerStarted -node $server)
		{
			...
			if($clientState -eq "TestCompleted" -and $clientSummary -eq "PASS")
			{
				...
				$suppressedOut = RunLinuxCmd -username $user -password $password -ip $dtapServerIp -port $dtapServerSshport -command "$python_cmd check-server.py && mv Runtime.log check-server.py.log -f" -runAsSudo
