...
#        Set-Variable -Name allVMData -Value $allVMData -Scope Global
		...
		{
			LogMsg "Waiting $($currentTestData.InitialWaitSeconds) seconds to finish RDMA update."
			sleep -Seconds $($currentTestData.InitialWaitSeconds)
		}
		...
		else
		{
            ...
            $pingPongTestIntraNodeTestOut = RunLinuxCmd -ip $serverVMData.PublicIP -port $serverVMData.SSHPort -username $user -password $password -command "/opt/intel/impi/2017.3.196/intel64/bin/mpirun -hosts server-vm -ppn 2 -n 2 -env I_MPI_FABRICS dapl -env I_MPI_DAPL_PROVIDER=ofa-v2-ib0 /opt/intel/impi/2017.3.196/intel64/bin/IMB-MPI1 pingpong > /home/$user/pingPongTestIntraNodeTestOut.txt 2>&1" -ignoreLinuxExitCode
            $pingPongTestInterNodeTestOut = RunLinuxCmd -ip $serverVMData.PublicIP -port $serverVMData.SSHPort -username $user -password $password -command "/opt/intel/impi/2017.3.196/intel64/bin/mpirun -hosts server-vm,client-vm -ppn 2 -n 4 -env I_MPI_FABRICS dapl -env I_MPI_DAPL_PROVIDER=ofa-v2-ib0 /opt/intel/impi/2017.3.196/intel64/bin/IMB-MPI1 pingpong > /home/$user/pingPongTestInterNodeTestOut.txt 2>&1" -ignoreLinuxExitCode
            #region EXECUTE TEST
